    ...
    <div className="animate-fade-in group relative rounded-4xl border transition-all duration-500
        ...
        <div className="p-6 pt-2">
            <div className="flex justify-between items-end mb-6">
                <div>
                    <span className="text-xs font-bold text-cyan-600 dark:text-cyan-400 uppercase tracking-tighter">{car.category}</span>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-none mt-1">{car.name}</h3>
                </div>
                <div className="text-right">
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold">Price</p>
                    <p className="text-lg font-black text-slate-900 dark:text-white">{car.price}</p>
                </div>
            </div>
            ...
            <div className="flex justify-between items-center p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 mb-6">
                <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400">{car.speed}</span>
                </div>
                <div className="w-[1px] h-6 bg-slate-200 dark:bg-white/10" />
                    <div className="flex flex-col items-center gap-1">
                        <FaGasPump className="text-cyan-500" size={16} />
                        <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400">{car.fuel}</span>
                    </div>
                <div className="w-[1px] h-6 bg-slate-200 dark:bg-white/10" />
                <div className="flex flex-col items-center gap-1">
                    <GiGearStickPattern className="text-cyan-500" size={18} />
                    <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400">{car.transmission}</span>
                </div>
            </div>
